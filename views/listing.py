...
def view_vm(uuid):
    ...
    if conn:
        try:
            ...
            # Use INACTIVE XML to see configuration (boot order, distinct devices)
            ...
            for disk in tree.findall('devices/disk'):
                disk_data = {
                    'device': disk.get('device'), # disk, cdrom
                    'file': 'N/A',
                    'target': 'N/A',
                    'type': disk.get('type')
                }
            ...
            if info[0] == libvirt.VIR_DOMAIN_RUNNING:
                try:
                    # Try to get IPs from DHCP leases
                    live_dom = conn.lookupByUUIDString(uuid) # Re-lookup might be needed for live state
            ...
            # If no specific device has boot order, check <os><boot dev='hd'/>
...
def update_boot_order(uuid):
    ...
    if conn:
        try:
            ...
            # Edit the CONFIG (Inactive XML)
            xml_str = dom.XMLDesc(libvirt.VIR_DOMAIN_XML_INACTIVE)
            tree = ET.fromstring(xml_str)
            
            # 1. Strip ALL old boot info
            ...
            def apply_order(selection, num):
                ...
                if type_ == 'disk':
                    ...
                    # Add boot order to first interface found
...
def console_vm(uuid):
    ...
    if not port or port == '-1':
        return "<h1>VM Not Running</h1><p>Start VM first.</p><a href='/list'>Back</a>"
